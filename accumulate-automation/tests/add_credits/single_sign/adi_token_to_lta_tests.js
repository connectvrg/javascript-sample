...
describe('Credits from ADI Token Account to Lite Token Account', function () {
  describe('Single Signature', function () {
    this.timeout(90000);

   
    it('[] Verify successful credits transaction from ADI Token Account to Lite Token Account with insufficient “max acme to spend” value', function () {
    ...
    it('[] Verify successful credits transaction from ADI Token Account to Lite Token Account with sufficient “max acme to spend” value', function () {
      const adiTokenAccount = adiTestData.adiAccounts.same_bvn.adiToken.account2;
      const liteTokenAccount = liteTestData.liteAccounts.same_bvn.account2;
      let no_of_credits = 5;
      let max_acme = 10;
    ...
    it('[] Verify successful credits transaction from ADI Token Account to Lite Token Account', function () {
      const adiTokenAccount = adiTestData.adiAccounts.same_bvn.adiToken.account2;
      const liteTokenAccount = liteTestData.liteAccounts.same_bvn.account2;
      let no_of_credits = 5;
  ...
  function failed_transfer(senderAccount, receiverAccount, no_of_credits, max_acme=0){
    const old_credits = commonLib.get_credits(receiverAccount.url);
        ...
        const new_credits = commonLib.get_credits(receiverAccount.url);

...
describe('ADI from Lite Token Account ', function () {
  describe('', function () {
    this.timeout(130000);
    ...
    it('[] Verify ADI Creation without sufficient credits', function () {
      ...
      liteObj.add_tokens(liteAccount.url, liteAccount.url);
      new addCredits().add_credits(liteAccount.url, liteAccount.url, 100);
      sleep(20000);

      const new_adi_url = "a.acme";
      let output = new adiCreate().adi_from_lite_account(liteAccount.url, new_adi_url, liteAccount.publicKey);
  ...
  function successful_creation(liteAccount, key_name = "", bookName = "") {
    ...
    // addCreditsObj.add_credits(liteAccount.url, liteAccount.identity, 600, 600);
    sleep(20000);
